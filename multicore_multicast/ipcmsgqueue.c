 ...
 * Return -2 if timeout.
 */
int mmcast_receive_timeout(struct ipc_message *ipc_msg, int s) {
   int recv_size;
   struct timer T;
   ...
   while (recv_size != s) {
      ...
      // mtype shall always be set to 1
...
/* receive a message and return its size */
int mmcast_receive(struct ipc_message *ipc_msg, int s) {
   int recv_size;
...
void mmcast_send(struct ipc_message ipc_msg, int s, int c) {
   int r;
