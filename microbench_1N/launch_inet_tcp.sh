...
for e in 0 1 2; do
   ./profiler/parser-sampling /tmp/perf.data.* --c 0 --base-event ${e} --app inet_tcp_microb > $OUTPUT_DIR/perf_producer_event_${e}.log
   ./profiler/parser-sampling /tmp/perf.data.* ${str} --base-event ${e} --app inet_tcp_microb > $OUTPUT_DIR/perf_consumers_event_${e}.log
done
