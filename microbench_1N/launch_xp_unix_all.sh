...
NUM_DGRAMS_ARRAY=( 10 1000 100000 )
...
for num_dgrams in ${NUM_DGRAMS_ARRAY[@]}; do
	...
	for num_consumers in ${NUM_CONSUMERS_ARRAY[@]}; do
		...
		for i in $(seq 0 $(( ${#TRANSFER_SIZE_ARRAY[@]}-1 )) ); do
			...
			echo "===== $num_consumers consumers, ${TRANSFER_SIZE_ARRAY[$i]}GB transfered, msg size is ${MSG_SIZE_ARRAY[$i]}B, $num_dgrams dgrams ====="
			./launch_unix.sh $num_consumers ${MSG_SIZE_ARRAY[$i]} $(( ${TRANSFER_SIZE_ARRAY[$i]} * 1000000000 )) $num_dgrams
