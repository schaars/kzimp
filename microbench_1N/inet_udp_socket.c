...
int IPC_receive(int msg_size, long *msg_id, uint64_t *spent_cycles)
{
  ...
  int recv_size = 0;
  while (recv_size < MIN_MSG_SIZE)
  {
    rdtsc(cycle_start);
    recv_size += recvfrom(sock, msg + recv_size, MIN_MSG_SIZE - recv_size, 0,
        0, 0);
    rdtsc(cycle_stop);
  ...
  while (recv_size < msg_size_in_msg)
  {
    rdtsc(cycle_start);
    recv_size += recvfrom(sock, msg + recv_size, msg_size_in_msg - recv_size,
        0, 0, 0);
    rdtsc(cycle_stop);
