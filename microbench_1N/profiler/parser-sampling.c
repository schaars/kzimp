...
static void check_host(struct mmaped_file *f)
{
  ...
  else
  {
    ...
    if (strcmp(host, hostname))
      printf(
          "#WARNING: HOSTS DIFFER (%s vs %s), functions names will likely be wrong\n",
          hostname, host);
    free(hostname);
  }
    ...
    if (event->header.type == PERF_RECORD_CUSTOM_EVENT_EVENT)
    {
      ...
      if (seen_events[nb_evts].new_event.name[0] == 0)
      {
        ...
        printf(
            ...
            nb_evts, event->new_event.name,
            (long long unsigned) event->new_event.event.config,
            (long long unsigned) event->new_event.event.sampling_period,
            (event->new_event.event.exclude_kernel) ? "yes" : "no",
            (event->new_event.event.exclude_user) ? "yes" : "no");
      }
      else
      {
        if (strcmp(seen_events[nb_evts].new_event.name, event->new_event.name)
            || event->new_event.event.config
                != seen_events[nb_evts].new_event.event.config
            || event->new_event.event.sampling_period
                != seen_events[nb_evts].new_event.event.sampling_period
            || event->new_event.event.exclude_kernel
                != seen_events[nb_evts].new_event.event.exclude_kernel
            || event->new_event.event.exclude_user
                != seen_events[nb_evts].new_event.event.exclude_user)
        {
...
static int check_samples(struct mmaped_file *f)
{
  ...
  for (; f->index < f->size;)
  {
    ...
    case PERF_RECORD_SAMPLE:
      ...
      if (event->ip.time < options.min_time || event->ip.time
          > options.max_time)
      {
        break;
      }
      ...
      if (event->ip.time < f->old_time[f->event])
      { /* Simple check : time should always go forward, otherwise this means that samples are mixed up. Should never happen. */
        fprintf(stderr, "#TIME BUG CORE %d EVT %d\n", f->event, core);
      }
      else if (options.temporal_dump)
      {
        if (event->ip.time - f->last_dump_time[f->event]
            > options.temporal_dump)
        {
          printf("%d\t%llu\t%llu\n", f->event,
              (long long unsigned) event->ip.time,
              (long long unsigned) f->samples_since_last_dump[f->event]);
      ...
      {
        int *value = g_hash_table_lookup(pid_hash[f->event][core], int_key(
            event->ip.pid));
        if (!value)
        {
          value = calloc(1, sizeof(*value));
          g_hash_table_insert(pid_hash[f->event][core], int_key(event->ip.pid),
              value);
        }
        *value = *value + 1;
      }
      ...
      {
        int *value = g_hash_table_lookup(tid_hash[f->event][core], int_key(
            event->ip.tid));
        if (!value)
        {
          value = calloc(1, sizeof(*value));
          g_hash_table_insert(tid_hash[f->event][core], int_key(event->ip.tid),
              value);
        }
        *value = *value + 1;

        value = g_hash_table_lookup(pid_hash[f->event][core], int_key(
            event->ip.tid));
        if (!value)
        {
          value = calloc(1, sizeof(*value));
          *value = event->ip.pid;
          g_hash_table_insert(pid_hash[f->event][core], int_key(event->ip.tid),
              value);
        }
      ...
      else if (options.ttid)
      {
        ...
        printf("%llu\t%d\t%d\t%d\t%s\n", (long long unsigned) event->ip.time,
            event->ip.tid, f->event, core, p ? p->name : "unknown");
      }
      else
      {
        f->old_time[f->event] = event->ip.time;
        int ret = symb_add_sample(&event->ip, (options.all_events) ? 0
            : f->event, core);
        if (ret == 4)
        { //ignored
          f->ignores++;
  ...
  if (f->index >= f->size && f->successes)
    printf(
        "#Finished parsing %s (%d events skipped and %d ignored and %d success - %d%%)\n",
        f->name, f->fails, f->ignores, f->successes, f->fails * 100 / (f->fails
            + f->successes));
...
static int get_lowest_time(struct mmaped_file *mmaped_files, int argc)
{
  ...
  for (i = 1; i < argc; i++)
  {
    if (mmaped_files[i].index >= mmaped_files[i].size)
      continue;
    sample_event_t *event =
        (sample_event_t *) &mmaped_files[i].data[mmaped_files[i].index];
...
int main(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    else if ((mmaped_files[file_index].data = mmap(0, sb.st_size, PROT_READ,
  ...
  printf("#RDT - Total duration of the bench %llu (%llu -> %llu)\n",
      (long long unsigned) (last_time - first_time),
      (long long unsigned) first_time, (long long unsigned) last_time);
  printf("#SAMPLES - Total duration of the bench %llu (%llu -> %llu)\n",
      (long long unsigned) (samples_last_time - samples_first_time),
      (long long unsigned) samples_first_time,
      (long long unsigned) samples_last_time);
  ...
  if (!options.ignore_samples && !options.temporal_dump && !options.pid
      && !options.tid && !options.ttid)
    symb_print_top();
