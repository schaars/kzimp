...
# compute the normalize value of the main summary
# against each summary in the list
def compute_normalize(main_summary, summaries):
   fd = open("plot.data", 'w')

   fd.write("#key")
   for s in summaries:
      fd.write("\tmean_normalize_%s/%s\tstdev_normalize_%s/%s"%(main_summary.name, s.name, main_summary.name, s.name))
   fd.write("\n")
...
if __name__ == "__main__":
   if len(sys.argv) >= 5:
      summaries = []
      summaries.append(Summary(sys.argv[2], sys.argv[3], sys.argv[1]))
      for i in xrange(4, len(sys.argv), 2):
         summaries.append(Summary(sys.argv[i], sys.argv[i+1])) 
      compute_normalize(summaries[0], summaries)
   else:
