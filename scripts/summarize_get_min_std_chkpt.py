...
class Experiment:
   ...
   def get_values_with_min_std(self, l):
      # compute the mean and stdev on the whole set of summaries
      a = scipy.array(l)
      m = a.mean()
      s = a.std()
      p = s * 100 / m

      if len(l) > self.nb_summaries_to_take:
         l.sort()
         for i in range(0, len(l)-self.nb_summaries_to_take+1):
            a = scipy.array(l[i:i+self.nb_summaries_to_take])
            m2 = a.mean()
            s2 = a.std()
            p2 = s2 * 100 / m2
               ...
               for nb_iter in self.all_nb_iter:
                  for channel_size in self.all_channel_size:
                     for limit_thr in self.all_limit_thr:
                        ...
                        for l in t:
                          (m, s, p) = self.get_values_with_min_std(l)
                          #a = scipy.array(l)
                          #m = a.mean()
                          #s = a.std()
                          #p = s * 100 / m
...
if __name__ == "__main__":
   if len(sys.argv) >= 3:
      E = Experiment(int(sys.argv[2]))
      for i in xrange(3, len(sys.argv)):
         E.add_summary(sys.argv[i])
      E.get_mean_stdev_summary(sys.argv[1])
   else:
      print("Usage: %s <out_summary_file> <nb_summaries_to_take> <in_summary_file1> [ ... <in_summary_filen>]"%(sys.argv[0])) 
