    ...
    for (i=0; i< perfmon_numArchEvents; i++)
    {
        if (biseqcstr(event_str, eventHash[i].name))
        {
            *event = eventHash[i];
            ...
            if (perfmon_verbose)
            {
                fprintf(OUTSTREAM,"Found event %s : Event_id 0x%02X Umask 0x%02X \n",
...
int
perfmon_setupEventSetC(char* eventCString, char*** eventnames)
{
     int i;
     ...
     (*eventnames) = (char**) malloc(perfmon_set.numberOfEvents * sizeof(char*));
