...
(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
  ...
  if (chan->compute_checksum)
  {
    ...
    {
      //printk(KERN_DEBUG "kzimp: process %i in read. Using smallbuff for a message of %lu\n", current->pid, count);
      m4chksum.checksum = oneC_sum(oneC_sum(0, &m4chksum, KZIMP_HEADER_SIZE), m->smallbuff, count);
    }
  }
...
(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
  ...
  else
  {
    ...
    {
      // construct the header
      memset(m, 0, KZIMP_HEADER_SIZE);
      m->len = count;
      m->bitmap = 0;

      m->checksum = oneC_sum(oneC_sum(0, m, KZIMP_HEADER_SIZE), m->smallbuff, count);
    }
  }
  m->data = kb; // this is correct in all the cases because we initialize kb at NULL
  m->len = count;
